# Gate Model (MobileNetV3-Small) - 데이터 분류 및 학습 결과 공유
...
원본 비율을 유지하는 **stratified split**입니다.
...
| `v1_pool.csv` (전체) | 12,976 | 11,868 | 1,108 |
...
| `v2_pool.csv` (전체) | 16,220 | 14,835 | 1,385 |
...
| `v3_pool.csv` (전체) | 19,464 | 17,802 | 1,662 |
...
|----------|---------|--------|---------|------|
| `v1_balanced_test.csv` | 400 | 200 | 200 | 1:1 균형 평가 |
| `v2_balanced_test.csv` | 400 | 200 | 200 | 1:1 균형 평가 |
| `v3_balanced_test.csv` | 400 | 200 | 200 | 1:1 균형 평가 |
| `v1_additional_test.csv` | 2,596 | 2,374 | 222 | 추가 데이터 전체 |
| `v2_additional_test.csv` | 3,244 | 2,967 | 277 | 추가 데이터 전체 |
| `v3_additional_test.csv` | 3,894 | 3,561 | 333 | 추가 데이터 전체 |
...
| 컬럼 | 설명 | 예시 |
|------|------|------|
| `path` | 이미지 절대 경로 | `/Users/.../v1/pretrain/normal/xxx.jpg` |
...
├── data/split_versions/
│   ├── v1/                          # 기본 증강
│   │   ├── pretrain/{normal,anomaly}/
│   │   └── additional/{normal,anomaly}/
│   ├── v2/                          # 중간 증강
│   └── v3/                          # 강한 증강
├── splits/                          # 학습/검증/테스트 split CSV
│   ├── v{1,2,3}_train_gate_mix.csv
│   ├── v{1,2,3}_val_gate.csv
│   ├── v{1,2,3}_test_gate.csv
│   ├── v{1,2,3}_train_normal.csv
│   ├── v{1,2,3}_val_mix.csv
│   ├── v{1,2,3}_test_mix.csv
│   └── v{1,2,3}_pool.csv
├── splits_eval/                     # 균형 평가셋
│   ├── v{1,2,3}_balanced_test.csv
│   └── v{1,2,3}_additional_test.csv
└── scripts/
    └── make_splits_v123.py          # 재생성 명령
```
...
| pos_weight | Auto (~10.7, normal/anomaly 비율) |
...
![v1 training curves](assets/v1_mnv3_small_training_curves.png)
...
![v2 training curves](assets/v2_mnv3_small_training_curves.png)
...
![v3 training curves](assets/v3_mnv3_small_training_curves.png)
...
![v1 confusion matrix](assets/v1_mnv3_small_test_cm.png)
...
![v2 confusion matrix](assets/v2_mnv3_small_test_cm.png)
...
![v3 confusion matrix](assets/v3_mnv3_small_test_cm.png)
...
| ROC (AUC=0.9992) | PR (AP=0.9842) |
|---|---|
| ![v1 roc](assets/v1_mnv3_small_test_roc.png) | ![v1 pr](assets/v1_mnv3_small_test_pr.png) |

#### v2

| ROC | PR |
|---|---|
| ![v2 roc](assets/v2_mnv3_small_test_roc.png) | ![v2 pr](assets/v2_mnv3_small_test_pr.png) |

#### v3

| ROC | PR |
|---|---|
| ![v3 roc](assets/v3_mnv3_small_test_roc.png) | ![v3 pr](assets/v3_mnv3_small_test_pr.png) |

---

## 4. Cascade Benchmark (Baseline vs Gate-Cascade)

`splits_eval/v{N}_balanced_test.csv` (400 images, 200:200) 기준, T_low=0.1, T_high=0.5
...
v1으로만 학습한 모델을 v2, v3 테스트셋에서 평가:

| 평가셋 | F1 | Accuracy |
|--------|-----|---------|
| v1_test_gate (same split) | 0.959 | 0.993 |
| v1_balanced_test | 0.993 | 0.993 |
| v2_test_gate (cross) | 0.943 | 0.990 |
| v2_balanced_test | 0.980 | 0.980 |
| **v3_test_gate (cross)** | **0.809** | 0.962 |
| v3_balanced_test | 0.965 | 0.965 |

**v3 test_gate에서 F1이 0.809까지 하락** -- 일반화 문제가 실제로 존재합니다.

### 5-4. 현재 진행 중

하이퍼파라미터 서치 (`scripts/hp_search_24h.py`)를 실행하여 다양한 정규화 전략 비교 중:
- Dropout 0.3~0.7
...
| 파일 | 위치 |
|------|------|
| Split CSV | `splits/v{1,2,3}_*.csv` |
| 평가용 Split | `splits_eval/v{1,2,3}_*.csv` |
| 학습 곡선 | `reports/assets/v{N}_mnv3_small_training_curves.png` |
| Confusion Matrix | `reports/assets/v{N}_mnv3_small_test_cm.png` |
| ROC Curve | `reports/assets/v{N}_mnv3_small_test_roc.png` |
| PR Curve | `reports/assets/v{N}_mnv3_small_test_pr.png` |
| 저장된 모델 | `models/v{N}_mnv3_small_gate.pt` |
| 모델 설정 | `models/v{N}_mnv3_small_config.json` |
| Split 생성 스크립트 | `scripts/make_splits_v123.py` |
| HP Search 스크립트 | `scripts/hp_search_24h.py` |
